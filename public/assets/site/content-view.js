(() => {
  ...
  const render=content=>{
    ...
    const photos=content.photos.moments.map(photo=>({...photo,url:displayPhotoURL(photo.url)})),next=JSON.stringify(photos);
    ...
    photos.forEach((photo,index)=>{
      ...
      if(/^\/assets\/site\/photos\/gallery-\d+-1000\.webp$/.test(photo.url)){img.srcset=`${photo.url.replace('-1000.webp','-640.webp')} 640w, ${photo.url} 1000w`;img.sizes='(max-width: 620px) 70vw, 350px';}
