(() => {
  ...
  const displayPhotoURL = url => url.replace(/^\/redesign(?:_v[1-6])?\/photos\//, '/assets/site/photos/');
  const photoKey = url => new URL(displayPhotoURL(url), location.href).href.replace(/-(?:640|1000)\.webp(?=$|\?)/, '.webp');
  ...
  const setPhotos = nextPhotos => {
    ...
    const safePhotos = nextPhotos.filter(photo => photo && typeof photo.url === 'string' && photo.url.trim()).map(photo => ({...photo, url: displayPhotoURL(photo.url)}));
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      const original = photo.url.match(/^\/assets\/site\/photos\/(gallery-\d{2})-1000\.webp$/);
      if (original && !existingImage) {
        ...
        const stem = `/assets/site/photos/${original[1]}`;
