  ...
  function setupScenes() {
    ...
    sceneContext = gsap.context(context => {
      if (!heroPlayed && window.scrollY < window.innerHeight) {
        heroPlayed = true;
        rememberStyles(document.querySelectorAll('.hero h1 > *, .hero-copy > p, .hero-actions, .hero-footnote, .photo-main, .photo-small, .photo-chess, .hero .ink-piece'));
        heroTimeline = gsap.timeline({ defaults: { ease: 'power3.out', clearProps: 'transform,opacity' } })
          ...
          .from('.photo-main', { y: 64, rotation: 0, opacity: .35, duration: .95 }, .1)
          .from('.photo-small', { y: 85, rotation: -12, opacity: .25, duration: .95 }, .19)
          .from('.photo-chess', { y: 72, rotation: 12, opacity: .25, duration: .95 }, .27)
          .from('.hero .ink-piece', { y: 28, rotation: -10, scale: .92, opacity: .15, duration: .8, stagger: .065 }, .32);
      }
      let setupFrame = 0;
