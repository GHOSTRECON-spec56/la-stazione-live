...
**Completed domain cutover on 14 September 2026:** `lastazionelb.com` and `www.lastazionelb.com` now use Cloudflare Worker Custom Domains, with the final V6 website and imported menu. The zone is Active. Keep this guide for maintenance and recovery; do not repeat the initial import or nameserver setup. See [production verification](cloudflare/VERIFICATION.md) for evidence, including successful Google authorized-origin checks on both domains. Production builds now use native Cloudflare headers, redirects and `/api/` endpoints. Code is on `migration/cloudflare`. Netlify is retired: its La Stazione project is disabled, its repository unlinked, its build hooks removed and its custom domain cleared. The historical setup and rollback instructions below are an archive, not steps to repeat. All owner links use https://lastazionelb.com/owner/ so Google receives the registered JavaScript origin.
