...
import {createGoogleIdTokenVerifier} from './google-verifier.mjs';
...
export function createGoogleSessions(storeFactory,{clientId=process.env.GOOGLE_CLIENT_ID||'',verifyIdToken,now=()=>Date.now()}={}) {
  ...
  const verify=verifyIdToken||createGoogleIdTokenVerifier(clientId);
  ...
  async function handleLogin(request,isAllowedEmail) {
    try {
      ...
      // Signature, audience, issuer and expiration are verified against Google's keys.
