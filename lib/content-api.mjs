...
export function validateContent(value) {
  ...
  for(const extra of m.extras){str(extra.label,'Extra group name',300,true);if(!groups.has(extra.groupId))fail('Extras refer to a missing menu group.');identifiers(extra.items,'Extras');for(const item of extra.items){str(item.name,'Extra name',500,true);price(item.usd,'Extra USD price');price(item.lbp,'Extra LBP price');}}
...
function checkWrite(request) {
  // Temporary open owner editing is intentional. This is CSRF protection, not sign-in.
  const origin=request.headers.get('origin');
  if(origin && origin!==new URL(request.url).origin)fail('Open the owner page on this website to publish.',403);
  if(request.headers.get('sec-fetch-site')==='cross-site')fail('Cross-site publishing is not allowed.',403);
}
export function createContentHandler(storeFactory) {
  return async request => {
    try {
      ...
      if(url.pathname.endsWith('/media')) {
        ...
        return reply({id:photoId,url:`/.netlify/functions/media?id=${photoId}`},201);
      }
      if(request.method==='GET'){
