...
## Netlify retirement and Google origin fix ? 14 September 2026

Deployed Cloudflare version `1f625666-8293-4d00-82fd-63f6892fef8d`. Owner requests from the workers.dev and www hosts redirect to https://lastazionelb.com before Google loads. Deployment ensures owner paths reach the Worker ahead of static assets. All 23 tests passed, including exact redirect destinations, local preview preservation, authorization and owner/editor permissions. Real browser clicks from all three entry points open Google sign-in without origin_mismatch. Signed-in multi-account selection still requires the user?s Google session.

Netlify project `glittery-pothos-c61d06` (`511c04ba-82a2-4f9e-a4f7-5428b49daf6c`) was unlinked through the Netlify API, custom_domain set to null, aliases emptied, and disabled. Read-back confirms disabled=true, repo_url=null, no domain aliases, and zero build hooks. Its public URL returns 404. The unrelated second Netlify project was left unchanged. Cloudflare public DNS, production menu, assets and storage remain active independently.
