...
const req=(method,body,headers={})=>new Request('http://localhost/.netlify/functions/content',{method,headers:{'Content-Type':'application/json',...headers},...(body?{body:JSON.stringify(body)}:{})});
...
test('Persistent publishing, concurrent write conflict, CSRF and upload validation',async()=>{
  const dir=await fs.mkdtemp(path.join(os.tmpdir(),'stazione-content-'));const store=localStore(dir);const handle=createContentHandler(()=>store);
  try {
    ...
    const other=createContentHandler(()=>localStore(dir));const read=await(await other(req('GET'))).json();assert.equal(read.content.menu.items[0].prices[0].usd,4.25);
    ...
    const invalid=await handle(new Request('http://localhost/.netlify/functions/media',{method:'POST',body:'<svg onload="alert(1)"></svg>'}));assert.equal(invalid.status,400);
    const bytes=await fs.readFile(new URL('../public/redesign_v4/photos/logo-160.webp',import.meta.url));
    const upload=await handle(new Request('http://localhost/.netlify/functions/media',{method:'POST',body:bytes}));assert.equal(upload.status,201);const photo=await upload.json();
