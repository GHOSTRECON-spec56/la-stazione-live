...
The owner's Google Client ID is deployed to Cloudflare as of 14 September 2026. The owner updated the authorized origins. Production browser checks now load the Google button without origin errors on both `https://lastazionelb.com` and `https://www.lastazionelb.com`. The website enforces Google authentication and the primary-owner/editor access list. Real owner sign-in and publishing should be verified interactively. The instructions below document the configuration for maintenance; do not create a second OAuth client unnecessarily. See [CLOUDFLARE-MIGRATION.md](CLOUDFLARE-MIGRATION.md).
